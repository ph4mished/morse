...
#TODO
#Help generation
#app name will be referenced from filename
#when help parameter == "" for all, do not generate help.
#when some arent == "" generate for whats available

#for required options or flags, when not set, output error
#Error: required flag(s) (flagnames) not set
#eg. Error: required flag(s) "file" not set
...
type
  ...
  Quantifier {.pure.} = enum
    #this necessarily means optional and required
    #[single, oneOrNone, oneOrMore, zeroOrMore,]# optionalSingle, requiredSingle, optionalMore, requiredMore
...
proc getOptionAssigner(s: Subcommand, key: string): Assigner =
  if key in s.longOptionAssigners:
    ...
    return (proc(value: string) {.closure.} = discard, Quantifier.requiredSingle)
  ...
  while true:
    ...
    of CmdTokenKind.empty:
      ...
      if currentSubcommand.index < len(currentSubcommand.argumentAssigners):
        ...
        for assigner in currentSubcommand.argumentAssigners[currentSubcommand.index..last]:
          if assigner.quantity in [optionalSingle, requiredSingle] and assigner.quantity != Quantifier.optionalSingle:
          #if assigner.quantity in [single, oneOrNone] and assigner.quantity != Quantifier.oneOrNone:
            #This makes it possible to use empty args for stdin
            # add argument name to make error message more understandble
            exitWithErrorMessage("Error: When a required argument is defined, a value must always be provided")

          if assigner.quantity in [requiredMore, optionalMore] and assigner.quantity != Quantifier.optionalMore:
            exitWithErrorMessage("Error: When required arguments are defined, values must always be provided")
      break
    ...
    of CmdTokenKind.argument:
      ...
      try:
        ...
        if assigner.quantity in [Quantifier.requiredSingle, Quantifier.optionalSingle]:
        ...
        #of Quantifier.requiredSingle, Quantifier.optionalSingle:
         # addToken(token)
        of Quantifier.optionalSingle, Quantifier.optionalMore:
          addToken(token)
        of Quantifier.requiredSingle, Quantifier.requiredMore:
      ...
      try:
        ...
        if peekCmdToken().kind == CmdTokenKind.argument:
          ...
          try:
            ...
            exitWithErrorMessage(getCurrentExceptionMsg())
        #elif assigner.quantity in [optionalSingle, requiredSingle] and assigner.quantity != Quantifier.optionalSingle:
         #   exitWithErrorMessage("Error: When a required argument is defined, a value must always be provided")
        elif token.value != "":
          # Conversion error
          exitWithErrorMessage(getCurrentExceptionMsg())
...
template argument*(identifier: untyped, t: typeDesc, required: bool=true): untyped =
  var identifier: t
  currentSubcommand.argumentAssigners.add((
    ...
    ,
    #if required: Quantifier.single else: Quantifier.oneOrNone
    if required: Quantifier.requiredSingle else: Quantifier.optionalSingle
  ))
...
template arguments*(identifier: untyped, t: typeDesc, atLeast1: bool=true): untyped =
  ...
  currentSubcommand.argumentAssigners.add(
    (
      ...
      if atLeast1: Quantifier.requiredMore else: Quantifier.optionalMore
    )
  )

#No need for long or short, use @[long, short, other_aliases]
template flag*(identifier: untyped; aliases: seq[string], required: bool=false): untyped =
  ...
  for alias in aliases:
    if alias.len > 1:
      currentSubcommand.longOptionAssigners[alias] = assigner
    else:
      currentSubcommand.shortOptionAssigners[alias] = assigner


template option*(identifier: untyped, t: typeDesc, aliases: seq[string],
                 default: t, required: bool=false): untyped =
...
template option*(identifier: untyped, t: typeDesc, aliases: seq[string], required: bool=false): untyped =
  var identifier: t
  var assigner = (
    proc(value: string) {.closure.} =
      try:
  ...
  for alias in aliases:
    #does long option and short options really make a difference here?
    #it seems it will work using only one option assigner
    if alias.len > 1:
      currentSubcommand.longOptionAssigners[alias] = assigner
    else:
      currentSubcommand.shortOptionAssigners[alias] = assigner
...
template exitoption*(aliases: seq[string], msg: string): untyped =
  var exiter = (
    (proc(value: string) {.closure.} = quit msg, QuitSuccess),
    Quantifier.requiredSingle
    #Quantifier.single
  )
  for alias in aliases:
    if alias.len > 1:
      currentSubcommand.longOptionAssigners[alias] = exiter
    else:
      currentSubcommand.shortOptionAssigners[alias] = exiter
...
template subcommand*(identifier: untyped, subcommandNames: seq[string],
